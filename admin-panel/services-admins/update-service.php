...
if (isset($_GET["upd_id"])) {
    ...
    // If no service found with that ID, redirect or show error
    if (!$service) {
        header("Location: show-services.php");
        exit();
    }
...
if (isset($_POST["submit"])) {
    ...
    // Fetch service data if not already loaded
    if (!$service) {
        $select = "SELECT * FROM services WHERE id = :id";
        $statement = $db->prepare($select);
        $statement->bindParam(":id", $id);
    ...
    if (empty($errors)) {
        try {
            ...
            $old_img_name = $service->image ?? null;
            
            // Dynamically build the SQL query
            $sql = "UPDATE services SET service_name = :name, description = :description";
            if ($image_uploaded) {
                $sql .= ", image = :img";
            }
            $sql .= " WHERE id = :id";
    ...
    <div class="container-fluid">
       <div class="row">
        <div class="col">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title mb-5 d-inline">Update Service</h5>
          <form method="POST" action="" enctype="multipart/form-data">
                ...
                <div class="form-outline mb-4 mt-4">
                  ...
                  <input type="text" name="name" id="service-name" class="form-control" placeholder="Service name" value="<?php echo htmlspecialchars($service->service_name ?? ''); ?>" required />
                </div>
                
                <!-- Description input -->
                <div class="form-outline mb-4">
                  <label for="description" class="form-label fw-bold">Description</label>
                  <textarea name="description" id="description" class="form-control" rows="4" placeholder="Service description" required><?php echo htmlspecialchars($service->description ?? ''); ?></textarea>
                </div>
                
                <!-- Image input -->
                <div class="form-outline mb-4">
                  <label for="image-upload" class="form-label fw-bold">Service Image</label>
